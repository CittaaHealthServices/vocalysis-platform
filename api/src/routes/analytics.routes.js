...
const getSessionScore = (session) =>
  session?.employeeWellnessOutput?.overallScore ??
  session?.employeeWellnessOutput?.wellnessScore ??
  null;
...
router.get('/overview', requireAuth, async (req, res) => {
  try {
    ...
    const [
      ...
      Alert.countDocuments({ tenantId, status: { $in: ['active', 'acknowledged', 'escalated'] } }),
      Alert.countDocuments({ tenantId, status: { $in: ['active', 'acknowledged', 'escalated'] }, alertLevel: 'critical' }),
      ...
      Session.aggregate([
        { $match: { ...query, tenantId, status: 'completed' } },
        {
          $group: {
            _id: null,
            avgScore: {
              $avg: { $ifNull: ['$employeeWellnessOutput.overallScore', '$employeeWellnessOutput.wellnessScore'] }
            }
          }
        }
      ])
  ...
  try {
    ...
    if (format === 'csv') {
      ...
      const rows = sessions.map(session => [
        ...
        getSessionScore(session) ?? 'N/A',
        session.employeeWellnessOutput?.stressLevel || session.employeeWellnessOutput?.wellnessLevel || 'N/A',
        session.status
      ]);
