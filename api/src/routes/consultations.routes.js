...
const normalizeConsultationMode = (mode) => (mode === 'inperson' ? 'offline' : mode);
...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    } else if (isClinicianRole(userRole)) {
...
router.post('/', requireAuth, requireRole(['HR_ADMIN', 'COMPANY_ADMIN', 'CLINICIAN']), async (req, res) => {
  try {
    ...
    // Validate mode
    if (!['online', 'inperson', 'offline'].includes(mode)) {
    ...
    const consultation = new Consultation({
      ...
      scheduledBy: userId,
      consultationType,
      mode: normalizeConsultationMode(mode),
    ...
    if (mode === 'online' && tenant?.googleConfig?.autoCreateMeetLinks && clinician.googleProfile?.refreshToken) {
      try {
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (isClinicianRole(userRole) && consultation.clinicianId._id.toString() !== userId.toString()) {
      return res.status(403).json({ error: 'Unauthorized' });
    }
