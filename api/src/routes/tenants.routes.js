...
const mapTenantResponse = (tenant) => ({
  id: tenant._id,
  ...
  usedAssessmentCount: tenant.usedAssessmentCount,
...
router.get('/', requireAuth, requireRole(['CITTAA_SUPER_ADMIN']), async (req, res) => {
  try {
    ...
    const [tenants, total] = await Promise.all([
      Tenant.find(query)
        .select('tenantId displayName legalName status monthlyAssessmentQuota usedAssessmentCount createdAt')
        .sort({ createdAt: -1 })
        .skip(skip)
    ...
    res.json({
      tenants: tenants.map(mapTenantResponse),
      pagination: {
        page: parseInt(page),
        limit: parseInt(limit),
        total,
...
router.post('/', requireAuth, requireRole(['CITTAA_SUPER_ADMIN']), async (req, res) => {
  try {
    ...
    const generatedTenantId = slugifyTenantId(name);
    const existingTenant = await Tenant.findOne({ tenantId: generatedTenantId });
    if (existingTenant) {
...
router.put('/:id', requireAuth, requireRole(['CITTAA_SUPER_ADMIN']), async (req, res) => {
  try {
    ...
    if (name && name !== tenant.displayName) {
      const nextTenantId = slugifyTenantId(name);
      const existing = await Tenant.findOne({ tenantId: nextTenantId, _id: { $ne: id } });
      if (existing) {
        return res.status(409).json({ error: 'Tenant name already exists' });
      }
      tenant.displayName = name;
      tenant.legalName = name;
      tenant.tenantId = nextTenantId;
    }
    ...
    await auditService.log({
      ...
      changeSnapshot: {
        before: originalData,
        after: { name: tenant.displayName, monthlyAssessmentQuota: tenant.monthlyAssessmentQuota, status: tenant.status }
      }
    });
