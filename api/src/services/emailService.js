...
function makeHeader(title, subtitle = '', gradientFrom = C.primary, gradientTo = '#2d6e60') {
  return `
  <div class="header" style="background: linear-gradient(140deg, ${gradientFrom} 0%, ${gradientTo} 100%);">
    <div class="logo-row">
      <span class="logo-mark">C</span>
      <span class="logo-name">Cittaa &nbsp;·&nbsp; ${BRAND_NAME}</span>
    </div>
    <h1>${title}</h1>
...
function makeFooter() {
  ...
  <div class="footer">
    ...
    <p>
      Sent with care by <span class="brand">Cittaa Health Services</span>
    </p>
    <p style="margin-top:4px;">
      Powered by <span class="brand">VocaCore™</span> &nbsp;·&nbsp;
      <a href="${PLATFORM_URL}">Open Platform</a>
    </p>
...
async function sendWelcomeEmail({ to, name, loginUrl, tempPassword, companyName }) {
  ...
  const html = wrapHtml(`
    ...
    <div class="body">
      ...
      <div class="card">
        <div class="card-row"><span class="lbl">Your email</span><span class="val">${to}</span></div>
        <div class="card-row"><span class="lbl">Temporary password</span>
          <span class="val" style="font-family:'Courier New',monospace; letter-spacing:1px;">${tempPassword}</span>
        </div>
        <div class="card-row"><span class="lbl">Organisation</span><span class="val">${companyName}</span></div>
      </div>
      ...
      <div class="btn-wrap">
        <a href="${loginUrl}" class="btn" style="background: linear-gradient(135deg, ${C.primary}, #2d6e60);">Sign in to Cittaa →</a>
      </div>

      <div class="divider"></div>
      <p class="copy" style="font-size:13px; color:${C.muted};">
