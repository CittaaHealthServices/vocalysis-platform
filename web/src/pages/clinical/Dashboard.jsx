...
export const ClinicalDashboard = () => {
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: stats?.riskBreakdown?.high || 0,
      icon: Users,
      color: 'bg-orange-50',
  ...
  return (
    <div className="space-y-8">
      {/* Welcome */}
      <div>
        <h1 className="text-3xl font-bold text-app mb-2">Good morning, Dr. Clinician</h1>
      ...
      <div className="flex gap-4 flex-wrap">
        ...
        <Button variant="secondary" onClick={() => navigate('/clinical/alerts')}>View Alerts</Button>
        <Button variant="secondary" onClick={() => navigate('/clinical/patients')}>Patient Registry</Button>
      </div>
            ...
            <div className="space-y-3">
              {sessions.slice(0, 5).map((session) => (
                <div key={session.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="font-medium text-app">{session.patient?.name || 'Patient'}</p>
                    <p className="text-sm text-gray-600">
                      {session.scheduledAt ? format(new Date(session.scheduledAt), 'HH:mm') : '—'}
                    </p>
                  </div>
                  <span className="text-xs px-2 py-1 bg-cittaa-100 text-cittaa-700 rounded capitalize">
          ...
          {alerts?.data && alerts.data.length > 0 ? (
            <div className="space-y-3">
              {alerts.data.slice(0, 5).map((alert) => (
                <div key={alert.id} className="flex items-center justify-between p-3 border-l-4 border-red-500 bg-red-50 rounded">
